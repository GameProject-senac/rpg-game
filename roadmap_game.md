...
**Foco Atual (Estado de parada):** Fase 2, Round 3, Fase A e o ajuste de XP dos comuns (§2.16) **completos e validados em campo**. Pico/abate proporcional para boss fica para a **fase de loot** (futuro, não iniciado — ver §7.3). Próxima sessão: **alinhamento de escopo antes de qualquer código novo** (ver §6) — nenhum próximo alvo está decidido ainda.
...
**Não alterado:** `recalcularAtributosEfetivos`, o broadcast `level_up`, `TABELA_NIVEL`/subida em loop (§2.15 pontos 2-3), `mobs`/`mobs.experiencia_dropada` (continuam não lidos), snapshot periódico (já gravava `experiencia`, ver §1.1).

**Fora de escopo, deliberadamente adiado (fase de loot, não implementado):** pico de abate proporcional para **boss**, XP com memória de dano por jogador (múltiplos jogadores batendo no mesmo boss), qualquer diferenciação comum/boss além do que já existe (boss ainda não existe como conceito de código — inimigos são só o comum hardcoded de §2.13/§7.3).
...
## 6. Estado Exato para Retomada (Próxima Sessão) — atualizado 2026-08-02

**Ponto de Parada Atual:** **Fase 2 COMPLETA** (Pacotes 1-5, §2.3-§2.7, mais Round 1 e Round 2 de correção pós-teste-de-campo, §2.8-§2.11) e **Round 3 (Inventário Clicável) COMPLETO** (§2.12) — fechados e validados em teste de campo. Por cima disso, o dono do projeto conduziu a **Fase A** — migração pro schema oficial da equipe (banco `jogo_pi`, `db/setup_banco.sql`, substitui `rpg_game`/`server/schema.sql` das seções históricas §2.3/§2.4) — em três passos, **todos concluídos e validados em campo em 2026-07-31**: A1 (schema novo), A2 (levantamento: achou spawn de inimigos sobrepostos e inventário quebrado contra o schema novo), A3 (adaptou o inventário — §2.14). A correção do **spawn sobreposto** (§2.13) também está fechada e validada. O objeto `ITENS` hardcoded, órfão desde a A3, foi removido de `server/server.js`. Em 2026-07-31 foi implementado um primeiro modelo de XP/nível (§2.15, histórico), com XP de morte fixo em 50. **Em 2026-08-02, o dono ajustou o modelo de XP dos inimigos comuns** (§2.16) — pico de abate removido, caminho único `dano_efetivo × 0.1` por golpe, `personagens.experiencia` migrada pra `DECIMAL(12,2)`, barra de XP na UI, e um bug de `NaN` (causado pela migração — `mysql2` devolve `DECIMAL` como string) foi encontrado e corrigido no mesmo dia. **Implementado e validado em campo (2026-08-02).** Parada de hoje é aqui.

O servidor lê identidade real do MySQL (personagem + inventário via `JOIN` com `Itens`, §2.14), aplica molde de classe + buff de nível + bônus de itens equipados (recálculo em memória, nunca persistido em `personagens`), grava snapshot periódico (posição/HP/**experiência**, §1.1)/disconnect/inventário, cura e concede invulnerabilidade autoritária no respawn, mantém população contínua de inimigos sem sobreposição (§2.13) e elimina jogadores fantasma/duplicados. O client seleciona personagem, conecta e envia `join` sozinho — nenhuma etapa depende de DevTools. O Phaser roda duas cenas em paralelo durante o combate: `ExploracaoCombate` (jogo) e `UIScene` — tela de inventário que abre/fecha (`Tab` ou clique, `inventory_open`/`inventory_close`), jogador estático e totalmente fora de combate enquanto aberta, abas verticais e itens de `tipo='Equipamento'` clicáveis com destaque visual verde/cinza de equipado, e agora barra de progresso de XP (§2.16).

1. **Banco já de pé:** schema oficial aplicado via `db/setup_banco.sql` (banco `jogo_pi` — 14 tabelas, ver nota em §2), incluindo `table_nivel` (agora lida pelo código, §2.15) e `personagens.experiencia` como `DECIMAL(12,2)` (§2.16) — nas duas frentes, máquina atual e script de setup, confirmadas idênticas. Usuário dedicado `rpg_app` configurado; credenciais em `server/.env` (git-ignorado, não commitar). `server/schema.sql`/`server/seed_teste.sql` (banco `rpg_game`) são o setup antigo, pré-migração A1 — não é mais o que está de pé. **Nota:** o banco foi re-semeado durante o teste de hoje (`setup_banco.sql` rodado de novo) — os 5 personagens de teste voltaram pro estado inicial do seed (`nivel=1, experiencia=0`), não é resíduo de bug.
2. **Próxima ação — alinhamento de escopo antes de qualquer código novo** — nenhum próximo alvo está decidido. Candidatos naturais: **loot de equipamento no mapa** (§7.3, feature nova — nunca existiu, tabela `map_items` já existe no schema mas não está ligada a nenhum código) — é nessa fase que entra o **pico de abate proporcional para boss** (§7.3, adiado conscientemente em §2.16), a **Fase 3 formal** (Level Design Avançado, Tilemaps & Spatial Partitioning, §3), ligar inimigos à tabela `mobs` (destravaria usar `mobs.experiencia_dropada`, adiado conscientemente em §2.15), ou qualquer item da lista de decisões adiadas (§7).
3. **Pendências que sobrevivem a tudo isso, sem bloquear nada** (detalhe completo em §7): feedback de `equip_item`/`unequip_item` recusado ainda é silencioso; seleção de personagem pode mostrar dado defasado (mitigado com F5); cura de respawn só persiste no banco por snapshot/saída, não no instante do `join`.
...
* **Tabela `classes`, tabela `map_items`/`mob_drops` (loot de mapa; renomeada de `itens_instanciados_mapa` no schema real da A1 — ver nota em §2), Redis/cache, anti-cheat de validação de posição** — adiados desde a spec original da Fase 2 (§9.1), continuam fora de escopo. As tabelas já existem no schema `jogo_pi`, mas nenhum código lê/escreve nelas ainda.
